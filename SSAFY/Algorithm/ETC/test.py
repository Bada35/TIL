...
def len_sort(li1, li2):  # li1:zero_can li2:zero_coor
    sorted_tuples = sorted(zip(li1, li2), key=lambda x: len(x[0]))

    # 정렬된 튜플을 다시 두 개의 리스트로 분리
    li1, li2 = zip(*sorted_tuples)

    # zip의 결과로 나온 튜플들을 다시 리스트로 변환 (필요하다면)
    li1 = list(li1)
    li2 = list(li2)
