...
NICKNAME = 'BUK02_KIMGAHEUN_OHGWANGHWI'
...
while True:
    ...
    for i in target_num:
        if balls[i][0] != -1:
            ...
            temp_angle = math.degrees(math.atan2(height, width))

            # 목적구가 흰 공을 중심으로 1사분면에 위치했을 때 각도를 계산
            if whiteBall_x <= targetBall_x and whiteBall_y <= targetBall_y:
                angle = 90.0 - temp_angle
            # 목적구가 흰 공을 중심으로 2사분면에 위치했을 때 각도를 계산
            elif whiteBall_x >= targetBall_x and whiteBall_y <= targetBall_y:
                angle = 270.0 + temp_angle
            # 목적구가 흰 공을 중심으로 3사분면에 위치했을 때 각도를 계산
            elif whiteBall_x >= targetBall_x and whiteBall_y >= targetBall_y:
                angle = 270.0 - temp_angle
            # 목적구가 흰 공을 중심으로 4사분면에 위치했을 때 각도를 계산
            elif whiteBall_x <= targetBall_x and whiteBall_y >= targetBall_y:
                angle = 90.0 + temp_angle
